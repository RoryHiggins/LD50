...
#define OD_LOG_LEVEL_DEFAULT OD_LOG_LEVEL_WARN
...
OD_API_C OD_ENGINE_CORE_MODULE const char* odLogLevel_get_name(uint32_t log_level);
OD_API_C OD_ENGINE_CORE_MODULE uint32_t odLogLevel_get_max(void);
OD_API_C OD_ENGINE_CORE_MODULE void odLogLevel_set_max(uint32_t log_level);

OD_API_C OD_ENGINE_CORE_MODULE struct odLogContext odLogContext_construct(const char* file, const char* function, uint32_t line);

OD_API_C OD_ENGINE_CORE_MODULE void odLog_log_variadic(struct odLogContext logger, uint32_t log_level, const char* format_c_str, va_list args);
OD_API_C OD_ENGINE_CORE_MODULE void odLog_log(struct odLogContext logger, uint32_t log_level, const char* format_c_str, ...) OD_API_PRINTF(3, 4);
OD_API_C OD_ENGINE_CORE_MODULE void odLog_assert(struct odLogContext logger, bool success, const char* expression_c_str);

OD_API_C OD_ENGINE_CORE_MODULE char* odDebugString_allocate(uint32_t size);
