...
odFile_read(struct odFile* file, void* out_buffer, uint32_t buffer_size, uint32_t* out_size);
OD_API_C OD_ENGINE_PLATFORM_MODULE bool odFile_write(struct odFile* file, const void* buffer, uint32_t size);

OD_API_C OD_ENGINE_PLATFORM_MODULE bool
odFilePath_read_all(const char* file_path, const char* mode, struct odAllocation* out_allocation, uint32_t* out_size);
OD_API_C OD_ENGINE_PLATFORM_MODULE bool
odFilePath_write_all(const char* file_path, const char* mode, const void* buffer, uint32_t size);
