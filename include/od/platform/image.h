...
OD_API_C OD_ENGINE_PLATFORM_MODULE bool odImage_allocate(struct odImage* image, uint32_t width, uint32_t height);
OD_API_C OD_ENGINE_PLATFORM_MODULE void odImage_release(struct odImage* image);
OD_API_C OD_ENGINE_PLATFORM_MODULE bool odImage_read_png(struct odImage* image, const void* src_png, uint32_t src_png_size);
OD_API_C OD_ENGINE_PLATFORM_MODULE void odImage_get_size(const struct odImage* image, uint32_t* out_opt_width, uint32_t* out_opt_height);
