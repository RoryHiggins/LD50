...
#define OD_LOG_GET_CONTEXT() odLogContext_construct(__FILE__, __func__, static_cast<int32_t>(__LINE__))

#if OD_BUILD_LOG
#define OD_LOG(LEVEL, ...) odLog_log(OD_LOG_GET_CONTEXT(), LEVEL, __VA_ARGS__)
#else
#define OD_LOG(...)
#endif
#define OD_CHECK(EXPR) odLog_check(OD_LOG_GET_CONTEXT(), EXPR, #EXPR)
#define OD_ASSERT(EXPR) odLog_assert(OD_LOG_GET_CONTEXT(), EXPR, #EXPR)
...
OD_API_C OD_CORE_MODULE OD_NO_DISCARD struct odLogContext
...
odLog_check(struct odLogContext logger, bool success, const char* expression_c_str);
OD_API_C OD_CORE_MODULE void
