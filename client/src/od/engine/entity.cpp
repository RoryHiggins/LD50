...
const char* odEntityCollider_get_debug_string(const odEntityCollider* collider) {
	...
	return odDebugString_format(
		...
		odBounds2f_get_debug_string(&collider->bounds));
}
bool odEntityCollider_check_valid(const odEntityCollider* collider) {
	...
	if (!OD_CHECK(collider->id >= 0)
		|| !OD_CHECK(odBounds2f_check_valid(&collider->bounds))) {
		return false;
	}
...
bool odEntityCollider_equals(const odEntityCollider* a, const odEntityCollider* b) {
	...
	if ((a->id != b->id)
		...
		|| (!odBounds2f_equals(&a->bounds, &b->bounds))) {
		return false;
	}
...
const char* odEntitySprite_get_debug_string(const odEntitySprite* sprite) {
	...
	return odDebugString_format(
		...
		odColorRGBA32_get_debug_string(&sprite->color),
		odBounds2f_get_debug_string(&sprite->texture_bounds),
		odMatrix4f_get_debug_string(&sprite->transform));
}
...
void odEntity_get_rect(const odEntity* entity, odRectPrimitive *out_rect) {
	if (!OD_DEBUG_CHECK(odEntity_check_valid(entity))
		|| !OD_DEBUG_CHECK(out_rect != nullptr)) {
		return;
	}

	*out_rect = odRectPrimitive{
