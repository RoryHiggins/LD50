...
bool odTest_run(int32_t filters, char const* opt_name_filter) {
	...
	OD_INFO("Running client c++ tests");
	if (filters & OD_TEST_FILTER_SLOW) {
		OD_INFO("Skipping slow client c++ tests");
	}

	if (opt_name_filter != nullptr) {
		OD_INFO("Skipping client c++ tests not matching name=\"%s\"", opt_name_filter);
	}
	...
	for (odTestSuite suite: test_suites) {
		OD_DEBUG("Starting client c++ test suite \"%s\"", suite.name);
		...
		for (int32_t i = 0; i < suite.tests_count; i++) {
			...
			if ((opt_name_filter != nullptr) && (strstr(test.name, opt_name_filter) == nullptr)) {
				OD_DEBUG("Skipping client c++ test \"%s\"", test.name);
				continue;
			}
			...
			if (opt_name_filter) {
				OD_INFO("Starting client c++ test \"%s\"", test.name);
			} else {
				OD_DEBUG("Starting client c++ test \"%s\"", test.name);
			}
			...
			if (new_errors) {
				OD_ERROR("Failed client c++ test \"%s\"", test.name);
				return false;
			}

			OD_DEBUG("Successfully completed client c++ test \"%s\"", test.name);
		}

		OD_DEBUG("Successfully completed client c++ test suite \"%s\"", suite.name);
	}
