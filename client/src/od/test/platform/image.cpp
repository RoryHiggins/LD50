...
OD_TEST(odTest_odImage_init_png) {
	...
	odImage image;
	OD_ASSERT(odImage_init_png(&image, static_cast<const void*>(png), png_size));
	OD_ASSERT(odImage_begin(&image) != nullptr);
...
OD_TEST(odTest_odImage_read_invalid_png_fails) {
	...
	odImage image;
	{
		...
		OD_ASSERT(!odImage_init_png(&image, static_cast<const void*>(invalid_png), invalid_png_size));
	}
}
