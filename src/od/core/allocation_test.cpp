...
TEST(odAllocation, swap) {
	...
	ASSERT_TRUE(odAllocation_get(&allocation1) != nullptr);
	ASSERT_TRUE(odAllocation_get(&allocation2) != nullptr);
	...
	ASSERT_TRUE(odAllocation_get(&allocation1) == allocation2_old_ptr);
	ASSERT_TRUE(odAllocation_get(&allocation2) == allocation1_old_ptr);
}
...
TEST(odAllocation, release) {
	...
	ASSERT_TRUE(odAllocation_get(&allocation) != nullptr);
