...
bool odImage_copy(odImage* image, const odImage* src_image) {
	...
	uint32_t size = sizeof(odColor) * src_image->width * src_image->height;
	if (size == 0) {
		return true;
	}
...
bool odImage_allocate(odImage* image, uint32_t width, uint32_t height) {
	...
	uint32_t size = sizeof(odColor) * width * height;
	if (size == 0) {
		return true;
	}
...
bool odImage_read_png(odImage* image, const void* src_png, uint32_t src_png_size) {
	...
	if (!OD_ASSERT(PNG_IMAGE_SIZE(png) == (width * height * sizeof(odColor)))) {
		return false;
	}
